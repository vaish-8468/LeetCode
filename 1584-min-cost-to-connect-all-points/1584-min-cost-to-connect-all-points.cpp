// class DSUnion{
//     private:
//     vector<int> root;
//     vector<int> rank;
    
//     public:
    
//     DSUnion(int size, vector<vector<int>>& points ){
//         for(int i=0;i<size;i++){
//             root.push_back(i);
//             rank.push_back(1);
//         }
//     }
    
//     int find(int x){
//         if(x==root[x]){
//             return x;
            
//         }
//         return root[x]=find(root[x]);
//     }
    
//     void Union(int x, int y){
//     int rootX=find(x);
//     int rootY=find(y);
    
//         if(rootX!=rootY){
    
//             //not connected
//             if(rank[rootX]>rank[rootY]){
//                 root[rootY]=rootX;
//             }
//             else if( rank[rootY]>rank[rootX]){
//                 root[rootX]=rootY;
//             }
//             else{
//                 root[rootY]=rootX;
//                 rank[rootX]++;
//             }
            
//         }
        
//     }
    
//     bool connected(int x, int y){
//         return find(x)==find(y);
//     }
// };
// class Solution {
// public:
//     int minCostConnectPoints(vector<vector<int>>& points) {
//         int n=points.size();
//         map<vector<int>,vector<vector<int>>> adj;
//         //each data point will store the connection and its corresponding distance between the nodes
        
//         DSUnion dsu(points.size(),points);
//         for(int i=0;i<points.size();i++){
//             int u=points[i][0];
//             int v=points[i][1];
//             for(int j=i+1;j<points.size();j++){
//                 int x=points[j][0];
//                 int y=points[j][1];
//                 //key->index, x, y
//                 //vector-> index, u, v, dist
//                 int dist=abs(u-x)+abs(v-y);
//                  adj[{i,u,v}].push_back({j,x,y,dist});
//                 adj[{j,x,y}].push_back({i,u,v,dist});
//             }
//             sort(adj[{i,u,v}].begin(), adj[{i,u,v}].end(), [&](const vector<int>& a, const vector<int>& b){
//                 return a[3]<b[3];
//             });
//             //  Connects each pair of points with a weighted edge, the weight being the manhattan distance between those points.
//         }
        
//         int cost=0;
//         int edgesUsed=0;  //should be equal to points.size()-1
       
        
//         for(auto node: adj){
//             for(auto conn: node.second){
//                 if(!dsu.connected(node.first[0],conn[0]) && edgesUsed<points.size()){
//                     dsu.Union(node.first[0],conn[0]);
//                     cost+=conn[3];
//                     edgesUsed++;
//                     break;
//                 }
//             }
//         }
        
//         return cost;
        
        
//     }
// };

// class UnionFind {
// public:
//     vector<int> group;
//     vector<int> rank;

//     UnionFind(int size) {
//         group = vector<int>(size);
//         rank = vector<int>(size);
//         for (int i = 0; i < size; ++i) {
//             group[i] = i;
//         }
//     }

//     int find(int node) {
//         if (group[node] != node) {
//             group[node] = find(group[node]);
//         }
//         return group[node];
//     }

//     bool join(int node1, int node2) {
//         int group1 = find(node1);
//         int group2 = find(node2);
        
//         // node1 and node2 already belong to same group.
//         if (group1 == group2) {
//             return false;
//         }

//         if (rank[group1] > rank[group2]) {
//             group[group2] = group1;
//         } else if (rank[group1] < rank[group2]) {
//             group[group1] = group2;
//         } else {
//             group[group1] = group2;
//             rank[group2] += 1;
//         }

//         return true;
//     }
// };

class UnionFind {
public:
    vector<int> group;
    vector<int> rank;

    UnionFind(int size) {
        group = vector<int>(size);
        rank = vector<int>(size);
        for (int i = 0; i < size; ++i) {
            group[i] = i;
        }
    }

    int find(int node) {
        if (group[node] != node) {
            group[node] = find(group[node]);
        }
        return group[node];
    }

    bool join(int node1, int node2) {
        int group1 = find(node1);
        int group2 = find(node2);
        
        // node1 and node2 already belong to same group.
        if (group1 == group2) {
            return false;
        }

        if (rank[group1] > rank[group2]) {
            group[group2] = group1;
        } else if (rank[group1] < rank[group2]) {
            group[group1] = group2;
        } else {
            group[group1] = group2;
            rank[group2] += 1;
        }

        return true;
    }
};


class Solution {
public:
    int minCostConnectPoints(vector<vector<int>>& points) {
        int n = points.size();
        vector<pair<int, pair<int, int>>> allEdges;
        
        // Storing all edges of our complete graph.
        for (int currNode = 0; currNode < n; ++currNode) {
            for (int nextNode = currNode + 1; nextNode < n; ++nextNode) {
                int weight = abs(points[currNode][0] - points[nextNode][0]) + 
                             abs(points[currNode][1] - points[nextNode][1]);
                
                allEdges.push_back({ weight, { currNode, nextNode }});
            }
        }
        
        // Sort all edges in increasing order.
        sort(allEdges.begin(), allEdges.end());
        
        UnionFind uf(n);
        int mstCost = 0;
        int edgesUsed = 0;
        
        for (int i = 0; i < allEdges.size() && edgesUsed < n - 1; ++i) {
            int node1 = allEdges[i].second.first;
            int node2 = allEdges[i].second.second;
            int weight = allEdges[i].first;
            
            if (uf.join(node1, node2)) {
                mstCost += weight;
                edgesUsed++;
            }
        }
        
        return mstCost;
    }
};